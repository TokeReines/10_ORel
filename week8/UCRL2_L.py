...
class riverswim:
    def __init__(self, nS, ergodic=False):
        self.nS = nS
        self.nA = 2
        ...
        if ergodic:
            for s in range(nS):
                if s == 0:
                    ...
                    self.P[s, 1, s] = 0.6
                    self.P[s, 1, s + 1] = 0.4
                    self.support[s][0] += [s, s+1]
                    self.support[s][1] += [s, s+1]
                elif s == nS - 1:
                    self.P[s, 0, s - 1] = 1
                    self.P[s, 1, s] = 0.6
                    self.P[s, 1, s - 1] = 0.4
...
class UCRL2_L:
    def __init__(self, nS, nA, delta=0.05, modified=False, support=None, use_support='none'):
        ...
        self.Nk = np.zeros(
            (self.nS, self.nA), dtype=int
        )  # Number of occurences of (s, a) at the end of the last episode.
        self.Nsas = np.zeros(
            (self.nS, self.nA, self.nS), dtype=int
        )  # Number of occureces of (s, a, s').
        self.Rsa = np.zeros((self.nS, self.nA))  # Cumulated reward observed for (s, a).
        self.vk = np.zeros(
            (self.nS, self.nA)
        )  # Number of occurences of (s, a) in the current episode.
    ...
    def confidence(self):
        ...
        for s in range(self.nS):
            for a in range(self.nA):
                n = max(1, self.Nk[s, a])
                if self.modified:
                    self.confR[s, a] = np.sqrt((3.5 * np.log((2 * self.nS * self.nA * t) / d)) / n)
                    self.confP[s, a] = np.sqrt((14 * self.nS * np.log((2 * self.nA * t) / d)) / n)
                else:
    ...
    def EVI(self, max_iter=2 * 10**2, epsilon=10 ** (-2)):
        ...
        action_noise = [
            (np.random.random_sample() * 0.1 * min((1e-6, epsilon)))
        ...
        V0 = np.zeros(
            self.nS
        )  # NB: setting it to the bias obtained at the last episode can help speeding up the convergence significantly!
        V1 = np.zeros(self.nS)
...
class UCRL2_L_SuppSize(UCRL2_L):
    def confidence(self):
        d = self.delta
        for s in range(self.nS):
            for a in range(self.nA):
...
class UCRL2_L_support(UCRL2_L_SuppSize):
    def max_proba(self, sorted_indices, s, a):
        # filter the sorted_indices      
        return super().max_proba(self.support[s][a], s, a)
...
# Plotting function.
def plot(data, names, y_label="Regret", exp_name="cumulativeRegret", q="0"):
    ...
    pl.ticklabel_format(
        axis="both", useMathText=True, useOffset=True, style="sci", scilimits=(0, 0)
    )
    ...
    for n in names:
        name += n + "_"
    pl.savefig("Q"+q + "_Figure_" + name + exp_name + ".pdf")
